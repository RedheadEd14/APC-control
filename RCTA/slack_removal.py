...
def slack_remove(sz, c):
		...
		T3_slack=True
		unwind_time=0.3
		deflection_small=0.025
		deflection_large=.05

		grav = sz.get_encoder_readings() # read GRAVITY components
		grav_comps = [grav[1],grav[0]]
		desired_comps = [0.0, 0.0]
		#STEP_1 remove slacks
		if (grav_comps[1]<0): # leaning away from T4
			...
			#Wind T4 till deflection sensed => 				T4 slack removed.
			grav_sensed_init = sz.get_encoder_readings() #read GRAVITY components
			#grav_sensed_init = [grav_sensed_init[1],-1 * grav_sensed_init[2],grav_sensed_init[0]] 

			pos_old = c.at.T4.get_pos()
			while(T4_slack):
				grav_sensed_curr = sz.get_encoder_readings() #read GRAVITY components 
				...
				if(grav_sensed_curr[1]-grav_sensed_init[1] > deflection_small):
					...
					break				
				c.at.T4.set_torque(0.1) 
				pos_new = c.at.T4.get_pos()
				change = measure_dtheta(sz,pos_new, pos_old)
				print "1Delta Theta is : %f"%change
				pos_old=pos_new
			c.at.T4.set_torque(0) #stop
		...
		elif (grav_comps[1]>0): # leaning towards T4

			if(grav_comps[0]<0): #leaning away from T2
				#unwind T4 and T3 a bit
				c.at.T4.set_torque(-0.1) #T4
				sleep(unwind_time)
			...
			#wind T4 till *small*deflection sensed =>		T4 slack removed
			grav_sensed_init = sz.get_encoder_readings() #read GRAVITY components
			#grav_sensed_init = [grav_sensed_init[1],-1 * grav_sensed_init[2],grav_sensed_init[0]] 
